...
interface NativeLanguageSelectorProps {
  onSelectNativeLanguage: (language: Language) => void;
  t: Translations;
}

const NativeLanguageSelector: React.FC<NativeLanguageSelectorProps> = ({ onSelectNativeLanguage, t }) => {
  return (
    <div className="flex flex-col items-center justify-center min-h-screen bg-white p-4">
      <div className="w-full max-w-md text-center">
        <h1 className="text-3xl font-extrabold text-slate-800 mb-2">{t.welcomeTo} MovLingo</h1>
        <p className="text-xl text-slate-500 mb-8">{t.selectNativeLanguage}</p>
        
        <div className="grid grid-cols-2 gap-4">
          {NATIVE_LANGUAGES.map((lang) => (
            <button
              key={lang.name}
              onClick={() => onSelectNativeLanguage(lang)}
              ...
              aria-label={`Select ${lang.name} as native language`}
            >
